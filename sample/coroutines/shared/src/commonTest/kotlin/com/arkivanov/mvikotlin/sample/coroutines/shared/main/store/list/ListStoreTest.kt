...
class ListStoreTest {
    ...
    fun loads_items_from_database_WHEN_created() {
        ...
        assertContentEquals(items, store.state.items.map(TodoItem::data))
    }
    ...
    fun adds_item_to_state_WHEN_Intent_AddToState() {
        ...
        assertContentEquals(listOf(oldItem, newItem), store.state.items)
    }
    ...
    fun deletes_item_from_state_WHEN_Intent_DeleteFromState() {
        ...
        assertContentEquals(listOf(item1, item3), store.state.items)
    }
    ...
    fun updates_item_in_state_WHEN_Intent_UpdateInState() {
        ...
        assertContentEquals(listOf(item1, item2.copy(data = newData2), item3), store.state.items)
    }
